...
export async function GET(req: Request) {
  try {
    ...
    // A query abaixo usa "FROM respostas_alunos ra": com alias, o Postgres so
    // aceita referencia pelo alias (ra.*). Usar respostasAlunos.* aqui gerava
    // "invalid reference to FROM-clause entry" e a tela de aluno quebrava.
    if (alunoId) conditions.push(sql`ra.aluno_id = ${alunoId}`);
    if (alunoNome) conditions.push(sql`ra.aluno_nome = ${alunoNome}`);
    if (turmaId) conditions.push(sql`ra.turma_id = ${turmaId}`);
