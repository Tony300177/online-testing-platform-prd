...
function BarGenero({ porGeneroGlobal }: { porGeneroGlobal: GeneroBreakdown[] }) {
  if (!porGeneroGlobal || porGeneroGlobal.length === 0) {
    return <p className="py-8 text-center text-sm text-slate-400">Sem dados de gênero.</p>;
  }
  const data = porGeneroGlobal.map((g) => ({
    sexo: g.sexo,
    acertos: g.acertos,
    erros: g.erros,
    naoRespondeu: g.naoRespondeu,
    pctAcerto: g.pctAcerto ?? 0,
    total: g.total,
  }));
  const dataMap = new Map(data.map((d) => [d.sexo, d]));
  return (
